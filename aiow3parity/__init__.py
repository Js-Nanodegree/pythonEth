...
class ETH:
    def __init__(self, currency_id, key, hw, host, log_function, passphrase, smart_contract,mnemonic, send_request_aiohttp):
        # ADD web3 interface init state
        ...
        # TODO ADD smart contract USDT take in config
        self.smart_contract = smart_contract
        self._key = key  # TODO ADD key hash for raw transaction take in config
        # TODO ADD key passphrase for create address take in config default ("123123")
    ...
    async def set_status_address(self, body):
        ...
        body = (1, 'parity_setAccountMeta', [
                address, str(json.dumps({key: value}))])
    ...
    async def _check_balance_usdt(self, address):
        ...
        try:
            ballance = contact.functions.balanceOf(
    ...
    async def _get_block(self):
        ...
        print(block['highestBlock']-block['currentBlock'],
              block['currentBlock'], self.ballance)
        return True
    ...
    def _build_contract_usdt(self):
        try:
            answer = self.web3.eth.contract(
                address=self.smart_contract, abi=abi)
        except Exception as msg:
            return None
        return answer
